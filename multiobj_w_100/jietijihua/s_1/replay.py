...
class ReplayBuffer:
    def __init__(self, max_len, state_dim, action_dim, if_on_policy, if_per, if_gpu):
        ...
        self.guiyi = StandardScaler()
        
        self.cunchu = np.empty([max_len,9])
    ...
    def sample_all(self) -> tuple:
        ...
        """
        # if len(self.cunchu) == 0:
        
        #     state_ori = self.buf_state[:self.now_len]
            
        #     all_other = torch.as_tensor(self.buf_other[:self.now_len], device=self.device)
            
        #     self.cunchu = np.vstack((self.cunchu,state_ori))
            
        #     state_out = self.guiyi.fit_transform(self.cunchu) 
        ...
        if len(self.cunchu) > 0:
            ...
            state_out = self.guiyi.transform(state_ori)
            ...
            shuliang = self.max_len * 100
            
            if len(self.cunchu) > shuliang:
                            
                self.cunchu = self.cunchu[-shuliang:,:]            
            
                        
            self.guiyi.fit(self.cunchu)
            
            # print('len(self.cunchu)',len(self.cunchu))
            
            # print('state_ori',state_ori)
        ...
        return (all_other[:, 0],
                ...
                all_other[:, 2:2 + self.action_dim],
                all_other[:, 2 + self.action_dim:],
                torch.as_tensor(state_out, device=self.device))
    
    def xuejiangeshu(self,geshu):
        
        self.cunchu = self.cunchu[:-geshu,:]   
        
        self.guiyi.fit(self.cunchu)
    
    
    def guiyi_initial(self):
